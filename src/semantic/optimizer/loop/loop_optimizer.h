...
public:
    ...
    void setEnablePartialUnrolling(bool enable) { enablePartialUnrolling_ = enable; }
...
private:
    ...
    bool enablePartialUnrolling_ = true;  // Enable partial unrolling with remainder
    ...
    // Clone with offset expression (for partial unrolling)
    StmtPtr cloneStatementWithOffset(Statement* stmt, const std::string& inductionVar, int64_t offset);
    ExprPtr cloneExpressionWithOffset(Expression* expr, const std::string& inductionVar, int64_t offset);
